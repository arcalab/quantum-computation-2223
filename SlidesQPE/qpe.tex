...
\begin{frame}{A Limitation of `Resolution'}
        ...
        We assumed that $\phi$ takes a value in $\left \{ \alert{0} \cdot
        \frac{1}{2^n}, \dots, \alert{2^{n-1}} \cdot \frac{1}{2^n} \right \}$

        Assumption arose from having only \alert{\underline{$n$ qubits}}
        available 

        What should we do if $\phi$ takes none of the values above?

        \pause
        Ideally: should return $\alert{k}$ where $\alert{k} \cdot \frac{1}{2^n}$
        is the value above closest to $\phi$

        \pause
        Can our QPE circuit do this?
\end{frame}

\begin{frame}{Setting the Stage}
        We will need an explicit definition for the
        inverse of QFT:
        \[
                \textstyle{
                QFT^{-1}_n \ket{x} = 
...
\}$ closest to $\phi$
        ...
        There exists $\epsilon$ s.t. $0 \leq | \epsilon | \leq
        \frac{1}{2^n}$ and $k \cdot \frac{1}{2^n} + \epsilon = \phi$

        Intuitively $\epsilon$ is an approximation error (which decreases
        when the number of qubits available increase)
...
\begin{frame}{Calculating \dots}
        \begin{flalign*}
            ...
            \textstyle{
                    ...
                    e^{i 2 \pi \alert{\epsilon} \cdot j } 
                    e^{i 2 \pi j \cdot \alert{\frac{1}{2^n}(k -l) } } \ket{l}
            }
            &
        \end{flalign*}
\end{frame}
...
\begin{frame}{An Analysis of the Final State}
        ...
        of measuring $\ket{k}$ let us study
        the corresponding amplitude in
        the final state
        \[
             \textstyle{
                    \frac{1}{2^n}
        ...
        Amplitude is $\textstyle{\frac{1}{2^n} \sum_{j = 0}^{2^n - 1} e^{i 2
        \pi \epsilon \cdot j } }$.  It is a \alert{finite} geometric series
        and therefore
        \[
        ...
        \pause
        We can safely proceed with the assumption that $\epsilon \not = 0$
\end{frame}

\begin{frame}{A Detour to Geometry}

        $| 1 - e^{i \theta}|$ (for some angle $\theta$) is the \alert{Euclidean
        distance} between 1 and $e^{i \theta}$ (\ie\ length of the
        \alert{straight line} segment between both points)

        Also useful to consider the \alert{arc length} $\theta$ between $1$ and
        $e^{i\theta}$ (\ie\ distance between the two points by running
...
\begin{frame}{Determining a Lower Bound}
        Recall that $\left |\frac{1}{2^n} 
        \frac{1 - e^{i 2 \pi \epsilon 2^n}}{1 - e^{i 2 \pi \epsilon}} \right |^2$
        is the probability of measuring $k$. Then
